...
\begin{examples*}
	...
	\item For \( \C \) locally small and \( A \in \Ob(\C) \), the \emdef{representable functor} \( h_A: \C \to \ctSet \), also written as
		\[
			\quad h_A(-) := \Hom_{\C}(A, -),
		\]
...
\begin{definition*}
	A functor \( F: \C_1 \times \C_2 \to \D \) is also called a \emdef{bifunctor}.
\end{definition*}
...
\begin{definition*}
	...
	Let \( F, G: \C \to \D \) be functors. A \emdef{natural transformation}
	\begin{equation*}
		\theta: F \To G
	\end{equation*}
...
If we fix \( \C \) and \( \D \), there is the \emdef{category of functors}
\begin{equation*}
	\ctFunc(\C, \D) = [\C, \D] = \D^{\C}.
\end{equation*}
And for functors \( F, G: \C \to \D \), there is a \emdef{category of natural transformations}
\begin{equation*}
	\ctNat(F, G) := [\C, \D](F, G).
\end{equation*}
...
\begin{definition*}
	Categories \( \C \) and \( \D \) are called \emdef{equivalent} if there are functors \( F: \C \to \D \), \( G: \D \to \C \) and natural isomorphisms \( \alpha: G \circ F \To \Id_{\C},~ \beta: F \circ G \To \Id_{\D} \). Notation: \( \C \simeq \D \).
\end{definition*}
