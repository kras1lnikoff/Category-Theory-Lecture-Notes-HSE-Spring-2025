...
\begin{definition*}
	A set of objects \( \{D_i\}_{i \in I} \) is called \emdef{weakly initial} if for all \( D \in \Ob(\ct{D}) \) there is \( i \in I \) and a map \( D_i \to D \).
\end{definition*}
...
\begin{definitions*}
	\item An \( \mrm{Ab} \)-category (a \emdef{preadditive} category) is a category \( \A \) such that
		\begin{enumerate}
			...
			\item The composition map \( \A(X, Y) \times \A(Y, Z) \to \A(X, Z) \) is bilinear:
				...
				\]
		\end{enumerate}
	\item If \( \A \) and \( \B \) are \( \mrm{Ab} \)-categories, a functor \( F: \A \to \B \) is called \emdef{additive} if
		\[
			\A(X, Y) \to \B(F(X), F(Y))
		\]
...
\begin{definition*}
	Let \( \A \) be an \( \mrm{Ab} \)-category and \( X, Y \in \A \). A \emdef{biproduct} of \( X \) and \( Y \) is a diagram
	\[
		\begin{tikzcd}
			X && {X \oplus Y} && Y
...
\begin{definition*}
	An \emdef{additive} category is an \( \mrm{Ab} \)-category with a zero object and binary biproducts.
\end{definition*}
