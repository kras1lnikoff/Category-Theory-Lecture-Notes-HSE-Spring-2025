...
\begin{definition*}
	An object \( Q \) is called \( F \)-\emdef{acyclic} if \( \LD_iF(Q)=0 \) for all \( i\ge 1 \).
\end{definition*}
...
\begin{definition*}
	For \( M\in \ctRMod \), the \emdef{projective dimension} \( \pdim_R(M) \) is the minimal integer \( n \) such that there exists a projective resolution
	\[
