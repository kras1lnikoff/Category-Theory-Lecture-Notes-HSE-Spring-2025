...
\begin{definition*}
	Functors \( F: \C \to \D \) and \( G: \D \to \C \) are called \emdef{adjoint} (\( F \) is \emdef{left adjoint} of \( G \) and \( G \) is \emdef{right adjoint} of \( F \); we write \( F \adj G \)) if there is a binatural bijection
	\[
